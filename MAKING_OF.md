...
## Opus 4.7 as development partner

The hackathon entry is built *about* AI-assisted reasoning. It's also built *with* it. Claude Opus 4.7 was the working partner throughout every layer of development: ideation, architecture, implementation, evaluation. This deserves a separate account, because the way the tool shaped the work is part of the story.

**Ideation.** The brief format — a JSON template with typed objectives, extraction schemas, meta-notice types, and suppression rules — came from a multi-session dialogue with Opus about what "adaptive interview" even means architecturally. The first instinct was to put everything in a system prompt. Opus surfaced the problem: a monolithic prompt rewards the model for doing everything adequately rather than any one thing well. The separation into four specialized calls emerged from that conversation. This was not prompt engineering; it was architecture design via dialogue.

**Domain embedding.** Each brief's `domain_context` field contains distilled axioms from the relevant field — memory science, VC failure taxonomy, Arnstein's Ladder. These were not written from scratch. They were produced by asking Opus to identify the five to ten most operationally-relevant insights from each domain for an interviewer who needs to probe *why* someone believes something, not just *what* they believe. Opus produced the initial drafts; we validated them against sources and trimmed to what would fit a system prompt cache block. The result is that the conductor operates with working knowledge of fields we don't personally have deep expertise in.

**Prompt calibration.** The conductor prompt has a constraint: no two questions in a single turn, no leading questions, no "that's great, now..." filler, no moves that give the participant the answer before they've had a chance to give it themselves. Getting these constraints to hold required iterative testing: running a session, identifying turns where the constraint was violated, going back to Opus to understand why the prompt structure allowed the violation, and tightening the rule. Opus was the partner in that diagnosis loop — not just the model being tested, but the reasoner explaining what the prompt didn't exclude.

**The meta-noticing audit rule.** The requirement that every candidate notice cite ≥2 distinct transcript turn indices wasn't in the first design. We noticed the meta-noticing layer was producing plausible-sounding observations that couldn't be anchored to anything specific — observations like "the participant seems defensive" with no turn-index evidence. The anchor rule emerged from asking Opus: "what's the weakest version of an observation you could produce that would still technically satisfy this prompt?" Opus produced a list. We plugged the holes.

**Evaluation harness.** We built an evaluation script (`scripts/eval-noticing.ts`) to test the meta-noticing layer against a corpus of synthetic sessions. The test cases — session transcripts designed to elicit specific notice types (source monitoring error, anchoring, hedge-drift) — were generated by Opus playing the role of a participant with a known reasoning pattern. The evaluator then ran meta-noticing against those sessions and measured recall and precision. Opus wrote both the test data and explained the failure modes in the evaluation output.

**Brief generation.** The most recent feature — an endpoint that generates a custom brief from a text description — is Opus 4.7 designing the interview structure for a domain it's never seen, using the brief format it helped us design. The full loop is: host describes their use case in plain language → Opus generates a complete brief JSON → the conductor runs it live in the next session. This is the system's strongest claim about generalization: the brief format is the stable interface; Opus generates conforming briefs on demand.

**What this means for the product.** The development experience surfaced something about what the platform could be. The interaction pattern of "here's a constraint, find the violation" and "here's an observation, find the anchor" is not just a prompt engineering exercise — it's the same pattern the meta-noticing layer uses at runtime. The meta-noticing prompt that asks Claude to find claims that don't cohere was written by a developer who had been in a dialogue where Claude found architectural claims that didn't cohere. Same cognitive operation; different layer.

---

## File counts and scope

This codebase: ~4,900 TS/TSX lines across 45 files, 14 commits over 2.5 days of active building.

The brief templates (three of them) are the rough equivalent of domain knowledge files: each embeds substantive calibrated axioms from the relevant field. The extraction schemas are the rough equivalent of data models for each interview domain.

The comparison that matters isn't files or LOC — it's whether the system does the thing it claims. We built a system that reads subtext in real time and adapts the conversation to it. That's the claim. The transcript database for any round is the evidence.
