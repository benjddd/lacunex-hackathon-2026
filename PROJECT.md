...
| D23 | **Voice: REVERSED AND SHIPPED.** | Cut formally on Thu; built Fri after the other commitments closed out. Groq Whisper large-v3 via `/api/transcribe`, MediaRecorder mic button in ChatPane (commit cf55adf). Falls back silently when `GROQ_API_KEY` is unset so local dev stays text-only. Text remains the primary path; voice is additive. |
...
| D27 | **Demo opens with a brief running**, not architecture | 0:00–0:30 is the interview + dashboard filling; architecture diagram is a 15–20s reveal mid-video. |
...
| D29 | `what_you_already_have_that_is_relevant` protected as closing frame | Differentiating section: names resources already in the participant's life they hadn't connected to the question. Prompt hardened with transcript-anchor examples. |
| D30 | **Dual naming: CaptainSubtext (internal/repo) + Lacunex (external/brand)** | ~~GitHub repo stays `captainsubtext` (established history, internal identifier).~~ All user-facing surfaces — README title, UI, submission form, video — use **Lacunex**. Docs that reference both should name both. Use **"cross-turn reasoning"** in descriptive copy. *(Supersedes the earlier "keep CaptainSubtext for UI" verdict; D1 locked Lacunex as the public brand Apr 24.)* **Repo-name clause superseded by D41 on Apr 24 — unified naming: repo renamed to `lacunex`.** |
...
| D35 | **Demo video news hooks locked** | Trio: Cloudflare Nov 2025 postmortem, Grenfell Phase 2, nuclear tacit-knowledge loss. |
...
| D45 | **Managed Agents — honest scope (one agent, not two)** (Apr 24) | Earlier notes and README claimed "two Managed Agents wired" (claim verifier + cohort synthesis). Audit: only the claim verifier uses the Managed Agents plane (`beta.agents` + `beta.environments` + `beta.sessions`, `web_search` tool, streamed events). Cohort synthesis is a single Messages-API call over N transcripts — a feature, not an agent. Rather than wrap the synthesis call in ceremonial agent machinery to justify a headcount, walked the dual-agent framing back across README, MAKING_OF, CALENDAR, submission. **Field 9 answer is still "Yes" — one genuine Managed Agent, with evidence the UI makes visible (live agent-event log on the session page).** Verified end-to-end Apr 24: 37s active container time, 4-claim fact-check, real web searches surfaced in UI. |
...
| D46 | **Demo scenario locked: civic consultation on a proposed congestion charge** (Apr 24-25) | Switched from founder-product-ideation to Civic Consultation to demonstrate domain-neutral adaptive reasoning (the platform's reach is not founder-only). Cold open: Cloudflare Nov 2025 + Grenfell Phase 2 inquiry headlines, pivot to "hours, not months" pre-investigation framing. Three new resident personas authored ([src/lib/personas.ts](src/lib/personas.ts)): `congestion_supporter_hidden_cost`, `congestion_opposed_values_air`, `small_shopkeeper_verifiable_claim`. 11-session cohort run + aggregate synthesised — see [docs/cohort/congestion-charge-2026-04-24.md](docs/cohort/congestion-charge-2026-04-24.md). Final 2:45 script: [DEMO_SCRIPT.md](DEMO_SCRIPT.md). |
| D47 | **Sim harness now persists conductor metadata** (Apr 25) | `scripts/simulate-session.ts` was dropping `move_type`, `anchor_turn`, `reasoning`, `deployed_notice`, `notice_candidates`, `objective_id` when persisting Turn objects — the data was returned by `/api/turn`, the sim just didn't capture it. Fixed: every saved sim session now renders `◆` and `↩` badges in `/sessions/[id]` without re-running meta-noticing. Also bumped `AGGREGATE_MAX_TOKENS` 8000 → 16000 after the first 11-session aggregate truncated mid-output. |
| D42 | **Credit-burn protection stack** (Apr 24) | Five layers. (1) Anthropic monthly cap — shared account with dev work, can't be lowered further. (2) Per-IP sliding-window limits on every LLM-touching route via `@upstash/ratelimit` on the existing Vercel KV — turn 40/10m, takeaway 20/1h, expensive (research/aggregate/synthesize) 5/1h, moderate (generate-brief/simulate/transcribe) 20/1h, light (invites) 60/1h. Fails open when KV creds absent (dev). (3) Per-invite turn budgets — `InviteRecord.turn_budget` default 40, `turns_used` incremented on each `/api/turn` with matching `inviteToken`; invite's `template_id` authoritative to prevent brief-swap drain. (4) Vercel Firewall rule `waf_G8cS9IgaDUJs` — denies `/api/*` with missing/empty `user-agent` at the edge (403 before function runs). Created via REST API `PUT /v1/security/firewall/config`. (5) Vercel Analytics for traffic-anomaly visibility. **Bypass for team testing:** `RATE_LIMIT_BYPASS_TOKEN` env var; requests carrying matching `x-bypass-token` header or `lacunex_bypass` cookie skip L2 + L3. Secret kept in Vercel prod env only, rotatable by changing the env var. All 429 responses include a human-readable `userMessage` so judges see "try again in N minutes" not "rate_limit_exceeded". |
...
|---|---|---|---|
| Q4 | Demo subject for recorded video | Resolved | **Civic Consultation on a proposed congestion charge** (D46 — switched from founder for competitor-distance + stronger thesis fit). Full 2:45 script in [DEMO_SCRIPT.md](DEMO_SCRIPT.md). 11-session cohort backing in [docs/cohort/](docs/cohort/). |
...
## 13. After the hackathon (2026-04-29)

**Hackathon over.** Submitted Sun Apr 26; judging concluded. About 20,000 people applied, 500 were selected to take part, and 288 projects were submitted and judged.

**Status:** this repository is the hackathon entry, kept as submitted. It stays MIT and public, and it is not maintained. Lacunex is a non-commercial project; nothing here is offered as a product or a service.

**Hosted demo:** switched off. A `DEMO_DISABLED=true` env var is honored by every cost-bearing route (`/api/turn`, `/api/takeaway`, `/api/generate-brief`, `/api/sessions/[id]/{generate-brief,research}`, `/api/rounds/[id]/{aggregate,synthesize,research}`, `/api/simulate-participant`, `/api/transcribe`) — they return 503 before any model call is made. See [src/lib/demo-gate.ts](src/lib/demo-gate.ts). The UI still renders so the artifact is browsable; the participant-side experience just doesn't run.
