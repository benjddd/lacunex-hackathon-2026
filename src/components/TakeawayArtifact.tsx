        ...
        <header className="flex shrink-0 items-center justify-between border-b border-stone-200 bg-white px-8 py-4">
          <div>
            <h2 className="text-xs uppercase tracking-widest text-stone-500">
              {isPreview ? "Your reflection — forming" : "Your reflection"}
            </h2>
            <p className="mt-0.5 text-sm text-stone-900">
              {isPreview
                ? "A draft that updates as you talk. Close this to return to the conversation."
                : "A reading of the conversation you just had."}
            </p>
            {isPreview && lastUpdatedTurn !== null && (
              <p className="mt-1 text-[11px] text-stone-400">
                Last refreshed at turn {lastUpdatedTurn}
              </p>
            )}
          </div>
          <button
          ...
          >
            {isPreview ? "Back to conversation" : "Close"}
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-10 py-8">
          {isGenerating && !markdown && (
            <div className="flex h-full flex-col items-center justify-center gap-3 text-stone-500">
              ...
              <p className="text-sm">
                {isPreview ? "Drafting your reflection…" : "Pulling the threads together…"}
              </p>
              <p className="text-xs text-stone-400">
                {isPreview
                  ? "A few seconds — this is a quick pass."
                  : "This takes up to a minute — one careful pass over the whole conversation."}
              </p>
            </div>
          )}
          {isGenerating && markdown && (
            <div className="mb-4 flex items-center gap-2 rounded-md bg-amber-50 px-3 py-2 text-[11px] text-amber-800">
              <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-amber-600" />
              Refreshing based on your latest turns…
            </div>
          )}
